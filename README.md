...
5. Generates `deploy/scripts/modules.manifest.json` and `sinfonia-apps.env`
...
2. Builds **Maestro** and **Sinfonia** Docker images from `apps/*/Dockerfile` (Sinfonia clients build in parallel; npm/native layers cache unless `package.json` changes)
...
```bash
./systemUp.sh                  # full build (uses Docker layer cache)
./systemUp.sh --skip-maestro   # SPA-only: reuse the existing Maestro image
./systemUp.sh --skip-frontend  # API-only: reuse existing Sinfonia images
./systemUp.sh --no-cache       # force a clean rebuild (slow)
```
...
│   ├── deploy.dockerignore
│   ├── maestro/
│   │   ├── Dockerfile     # Maestro image (Node 22, canvas, ffmpeg, opencv.js)
...
`systemUp.sh` copies `apps/deploy.dockerignore` to `deploy/.dockerignore` so git metadata and `node_modules` are not sent as build context. Vite `ARG`s are applied after `npm install`, so core/public/dyeus share dependency layers and only the Vite step differs.
